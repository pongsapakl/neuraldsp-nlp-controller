...
def _describe_tone(tone: CanonicalTone, preset_name: str) -> str:
    """Auto-generate a text description from preset name + canonical tone.

    All descriptors are derived from values the adapter extracted from the
    loaded plugin state — no stylistic claims (Vox, Fender, etc.) and no
    hand-written plugin-specific mappings. Honest measurements only.
    """
    parts = [preset_name]
